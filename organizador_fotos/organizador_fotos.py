...
def sanitizar_nome(nome: str) -> str:
    ...
    # strip() antes E depois do corte em 80 — turma com nome de professor
    # colado junto pode fazer o corte cair bem em cima de um espaço, o que
    # gera pasta terminando em espaço (o Windows esconde, o programa nao
    # sabe, e trava tentando escrever num caminho que ele acha que existe
    # mas nao existe mais desse jeito). Visto ao vivo em 22/09/2026.
    nome = nome.strip('. ')
    return nome[:80].strip('. ') or 'Aluno'
...
def organizar(pasta_origem: Path, pasta_saida: Path):
    ...
    for i, foto in enumerate(fotos, 1):
        ...
        try:
            ...
            # sem isso cairiam na pasta do ultimo aluno fotografado.
            ...
            # Alunos → Fichas avulsas). Nao tem nome real — so' um codigo. Em
            # vez de criar um caminho separado, vira um "aluno" temporario com
            # o proprio codigo como nome, e cai no fluxo normal abaixo (mesma
            # pasta, mesmos blocos). Depois que o admin vincular o codigo a um
            # aluno de verdade, é so' renomear esta pasta pelo nome real.
            ...
            if dados:
                ...
                fotos_aluno += 1
                # Foto de aluno leva o mesmo nome da pasta (aluno + turma), nao
                # o nome cru da camera (tipo 3G7A3181.JPG) — pedido real: sem
                # isso, quem olha a foto depois (ex: ja' dentro de _PRODUTOS ou
                # copiada pra fora da pasta) nao tem como saber de quem e'.
                # Foto de turma (aluno_atual None) continua com o nome original.
                if aluno_atual:
            ...
            # cv2 as vezes da erro interno numa foto especifica (QR borrado,
            # reflexo, angulo ruim) em vez de so' dizer "nao achei"; nome de
            # pasta gerado por dado ruim (turma com texto demais) tambem pode
            # gerar caminho invalido na hora de copiar. Sem isso, uma foto
            # problematica parava a leitura da pasta inteira sem avisar direito
            # onde. Visto ao vivo em 22/09/2026.
            print(f'❌ ERRO em {foto.name}: {e} — pulando essa foto')
